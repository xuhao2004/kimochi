  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const res = await fetch(`/api/account-change/revoke?${qs.toString()}`);
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="min-h-screen flex items-center justify-center p-6 relative z-10">
        <div className="w-full max-w-md text-center bg-white/80 backdrop-blur-xl border border-black/[0.06] p-8 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
          ...
          {status === 'ok' && (
            <>
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 mx-auto mb-4 flex items-center justify-center text-white">
                <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/></svg>
              </div>
              <h2 className="text-2xl font-semibold text-gray-900">撤回成功</h2>
              <p className="text-gray-600 mt-2">您的账号变更申请已成功撤回。</p>
              <a href="/login" className="inline-block mt-6 px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700">返回登录</a>
            </>
          )}
          {status === 'error' && (
            <>
...
export default function AccountChangeRevokePage() {
  return (
