...
export default function SCL90Page() {
  ...
  const initializeAssessment = async (authToken: string) => {
    try {
      ...
      const assessmentRes = await fetch("/api/assessments", {
        ...
        body: JSON.stringify({ type: "SCL90" })
      });
      ...
      // 2. 获取题目
      const questionsRes = await fetch("/api/assessments/questions?type=SCL90", {
      ...
      setOptions(questionsData.options);
  ...
  const handleSubmit = async () => {
    ...
    if (unansweredQuestions.length > 0) {
      alert(`还有 ${unansweredQuestions.length} 道题目未回答，请完成后再提交。`);
      return;
    }
    ...
    try {
      ...
      // 跳转到缓冲页面
      router.push(`/assessments/scl90/processing?id=${assessment.id}`);
        ...
        <div className="bg-white/80 backdrop-blur-md border-b border-black/[0.08] flex-shrink-0 z-20">
          <div className="max-w-4xl mx-auto px-6 py-4">
            <div className="flex items-center justify-between mb-3">
              <h1 className="text-xl font-semibold text-gray-900">SCL-90 心理症状自评量表</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                  {options.map((option) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500 mt-1">{option.value}分</div>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className={`px-8 py-3 rounded-xl font-medium transition-all duration-300 ${
                  !canGoNext() || submitting
                    ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                    : "bg-gradient-to-r from-rose-500 to-pink-500 text-white hover:from-rose-600 hover:to-pink-600 hover:scale-[1.02] shadow-sm"
                }`}
              >
                {submitting ? (
                  <div className="flex items-center space-x-2">
